...
/** Converts metres to kilometres for display. */
 ...
 * based on the unit's implied travel mode.
 */
export function travelDays(valueMeters: number, unit: UnitType): number | null {
  const speed = TRAVEL_SPEED_KM_PER_DAY[unit];
  if (!speed) return null;
  return valueMeters / 1000 / speed;
}

/** Returns the ordered list of reference IDs sorted by height ascending. */
export const HEIGHT_REF_IDS = Object.keys(HEIGHT_REFS).sort(
  (a, b) => HEIGHT_REFS[a].value - HEIGHT_REFS[b].value,
);

/** Returns the ordered list of distance reference IDs. */
